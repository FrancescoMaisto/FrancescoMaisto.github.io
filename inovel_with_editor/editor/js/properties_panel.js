...
// Add this helper function before displayProperties
...
// Function to create Choice block button in Properties Panel
function addNewChoiceButton(block, currentData) {
  // Only add the button if this is an Interactive paragraph
  if (block.dataset.type !== 'interactive') return;

  // Create button container
  const buttonContainer = document.createElement('div');
  buttonContainer.classList.add('new-choice-container');
  ...
  buttonContainer.appendChild(newChoiceButton);
  propertiesPanel.appendChild(buttonContainer);
}
...
function displayProperties(blockId) {
  ...
  togglePropertiesPanelVisibility(true);
  ...
  if (blockId === 'story') {
    ...
    `;
    if (currentData.story.stylesheet) {
      storyProperties += `${editableProperty}<strong>Style Sheet:</strong>${editableSpan}data-property="stylesheet">${currentData.story.stylesheet}</span></p>`;
    }
    if (currentData.story.typingSpeed) {
      storyProperties += `${editableProperty}<strong>Typing Speed:</strong>${editableSpan}data-property="typingSpeed">${currentData.story.typingSpeed}</span></p>`;
    }
    if (currentData.story.hasInventory !== undefined) {
  ...
  document.getElementById('closePropertiesPanel').addEventListener('click', () => {
    ...
    togglePropertiesPanelVisibility(false);
  });

  // Add event listeners to make properties editable
...
// Handles changes to the data object when properties are edited in the panel
function updateDataFromPanel(blockId, property, newValue) {
  // *******************************************************
  // STORY
  // *******************************************************
  if (blockId === 'story') {
    if (property === 'typingSpeed') {
  ...
  if (property === 'id') {
    ...
    const blockElem = document.querySelector(`.block[data-id="${blockId}"]`);
    if (blockElem) {
      const textLabels = Array.from(blockElem.getElementsByClassName('destination-label'));
      // Get the last one which is our text preview
      const textLabel = textLabels[textLabels.length - 1];
      if (textLabel) {
        textLabel.innerText = newValue.slice(0, 30) + "...";
      }
    }
  } else {
    currentData.story.chapters[chapterIndex].paragraphs[paragraphIndex][property] = newValue;
...
// Add this helper function for showing the confirmation dialog
...
// Update the deleteBlock function to use the confirmation dialog
function deleteBlock(blockId) {
  showConfirmationDialog(() => {
    ...
    if (blockId.startsWith('paragraph-')) {
      ...
      // Remove all choice blocks associated with this paragraph
    ...
    choiceConnections = choiceConnections.filter(conn => 
      conn.from !== blockId && conn.to !== blockId
    );
...
function togglePropertiesPanelVisibility(visible){
  const propertiesPanel = document.getElementById('propertiesPanel');
