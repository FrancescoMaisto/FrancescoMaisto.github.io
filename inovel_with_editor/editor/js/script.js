const jsonFileNames = {
  default: "editor/stories/il_vecchio_pozzo_TEST.json",
  new: "editor/stories/the_hobbit.json",
}
...
let choiceConnections = []; // Changed from const to let
let editor;
let svgCanvas;
let propertiesPanel;
...
function createFlowchart(currentData) {
  ...
  const storyX = currentData.story.position
    ? currentData.story.position[0]
    : 100 + (maxParagraphWidth - 120) / 2;
  ...
  // Remove all Story->Chapter connections
  // Instead, find Paragraph with id=0 and connect Story to it later
  let startingParagraph = null;
  ...
  chapters.forEach((chapter, cIndex) => {
    ...
    */
    ...
    chapter.paragraphs.forEach((para, pIndex) => {
      ...
      // Store reference to paragraph with id=0
      if (para.id === 0) {
      ...
      if (para.choices) {
        para.choices.forEach((choice, chIndex) => {
          ...
          // Create a grey connection from this choice block to the Paragraph block whose JSON id matches choice.destination_id
          const targetParaBlock =
            paragraphBlocksByJsonId[choice.destination_id];
          if (targetParaBlock) {
            choiceConnections.push({
              from: choiceId,
              to: getBlockId(targetParaBlock),
            });
          }
        });
      }
  ...
  // Create connection from Story to starting Paragraph (id=0)
...
function updateConnections() {
  ...
  connections.forEach((conn) => {
    ...
    if (fromBlock && toBlock) {
      // Remove chapter-specific logic and use story-paragraph for Story->Paragraph
      const connType =
        fromBlock.classList.contains("story")
          ? "story-chapter" // Reuse story-chapter style for Story->Paragraph
  ...
  choiceConnections.forEach((conn) => {
    ...
    const toBlock = blocks[conn.to];
    if (fromBlock && toBlock) {
      drawConnection(fromBlock, toBlock, "paragraph-choice", svgCanvas);
    }
  });
  ...
  // Draw PassThru->Paragraph connections
...
window.removeHighlightFromBlocks = function () {
  document.querySelectorAll(".block").forEach((block) => {
...
function createNewParagraph(type) {
  ...
  // Create visual block with flash enabled
  const blockId = `paragraph-0-${chapter.paragraphs.length - 1}`;
  const headerText = headerLabel.text[type];
  const block = createBlock(blockId, headerText, newParagraph.position[0], newParagraph.position[1], true);
...
function createTextBodyLabel(textBody, block){
  ...
  textbodyLabel.classList.add("destination-label");
  textbodyLabel.innerText = textBody.slice(0, 30) + "...";
