...
/*****************************************************************
 TEST STORY
 *****************************************************************/
document.getElementById("testStory").addEventListener("click", function () {
  // Open a new window
  ...
  // Wait for the new window to load, then send the story data
...
document.getElementById("saveStory").addEventListener("click", () => {
  ...
  currentData.story.chapters.forEach((chapter) => {
    chapter.paragraphs.forEach((para) => {
      ...
      if (paraBlock) {
        const editableElem = paraBlock.querySelector(
          '[data-property="text_body"]'
        );
        if (editableElem) {
  ...
  currentData.story.chapters.forEach(chapter => {
    chapter.paragraphs.forEach(para => {
      if (para.destination_id === undefined || 
          para.destination_id === null || 
          String(para.destination_id).trim() === "") {
        console.error("ATTENTION! There is an empty/null/undefined 'destination_id' in the data structure (currentData) that I had to remove. Make sure that empty/null/undefined 'destination_id's never get added to currentData in the first place!");
